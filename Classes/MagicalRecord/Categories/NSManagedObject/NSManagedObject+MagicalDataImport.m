...
{    
    for (NSString *attributeName in attributes) 
    {
        ...
        if(lookupKeyPath == nil){
            NSString* lookupKey = MR_attributeNameFromString(attributeName);
            NSString *val = [objectData valueForKeyPath:lookupKey];
            if(val != nil){
                lookupKeyPath = lookupKey;
            }
        }
...
- (NSDictionary*) MR_exportValues
{
    ...
    {
        NSString* lookupKey = MR_attributeNameFromString(attributeName);
        id val = [self valueForKeyPath:attributeName];
        if(val == nil){
            val = [NSNull null];
        }
        ret[lookupKey] = val;
...
- (NSDictionary*) MR_exportValues:(NSArray*)keys
{
